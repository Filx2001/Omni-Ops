// AI tool definitions + shared date helpers.
// Single source of truth — assistant.js and messageCreate.js both import from here
// so a tool can never exist in one place and be forgotten in the other.
const { parseLocalDateTime } = require("./dateParser");
...
function getZonedDateStr(str) {
  ...
  const res = parseLocalDateTime(datePart, timePart);
  return res?.date ? res.date.toISOString() : undefined;
}

/** Understands "today", "tomorrow" and "next week" in addition to normal dates */
function parseDueDate(input) {
  if (!input) return undefined;
  const lower = input.toLowerCase().trim();

  if (lower === "today" || lower === "tomorrow") {
    const res = parseLocalDateTime(lower, "11:59 pm"); // end of day, workspace tz
    return res?.date ? res.date.toISOString() : undefined;
  }
  if (lower === "next week") {
