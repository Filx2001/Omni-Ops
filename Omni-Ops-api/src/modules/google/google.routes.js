...
router.get("/callback", async (req, res) => {
  try {
    ...
    res.send(
      `<h1>✅ Google connected</h1><p><b>${email || ""}</b> — Sheets & Calendar are ready.<br>You can close this tab and return to your app.</p>`
    );
  } catch (err) {
    console.error("[Google] callback error:", err.message);
    res.status(500).send("❌ Connection failed. Try again from your app.");
  }
});
